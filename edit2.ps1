...
function ReplaceAll($find,$repl){
  $vF=$find; $vR=$repl; $T=$true; $F=$false; $wrap=1; $rep=2
  $fnd = $doc.Content.Find
  $fnd.ClearFormatting()
  $fnd.Replacement.ClearFormatting()
  return $fnd.Execute([ref]$vF,[ref]$T,[ref]$F,[ref]$F,[ref]$F,[ref]$F,[ref]$T,[ref]$wrap,[ref]$F,[ref]$vR,[ref]$rep)
}
...
try {
  ...
  $bulletStyle = $null
  try { $bulletStyle = $doc.Styles.Item([int]-49) } catch { L('bulletStyle lookup failed: ' + $_.Exception.Message) }
  $bullets = @($B1,$B2,$B3,$B4)
  foreach ($p in $doc.Paragraphs) {
    $t = ($p.Range.Text -replace "[`r`n`a]","").Trim()
    if ($t -eq $HEADING) {
       ...
       if ($bulletStyle -ne $null) { try { $p.Style=$bulletStyle } catch { L('set bullet style failed') } }
       $pr=$p.Range; $pr.Font.Name='Arial'; $pr.Font.Size=[single]10; $pr.Font.Bold=[int]0
       $p.SpaceBefore=[single]0; $p.SpaceAfter=[single]2; $p.LineSpacingRule=0
  ...
  $normal = $null
  try { $normal = $doc.Styles.Item([int]-1) } catch {}
  foreach ($p in $doc.Paragraphs) {
    $t = ($p.Range.Text -replace "[`r`n`a]","").Trim()
  ...
  # 5) Global: Arial, single spacing, bump <10 -> 10
  $bumped=0
  foreach ($p in $doc.Paragraphs) {
    $pr=$p.Range
  ...
  foreach ($p in $doc.Paragraphs) {
    ...
    if ($sz -eq 14 -and $bd -eq -1) {
       try {
         $bb=$pr.ParagraphFormat.Borders.Item([int]-3)
         $bb.LineStyle=[int]1; $bb.LineWidth=[int]6; $bb.Color=[int]5855577
         $bordered++
  ...
  $min=9999
  foreach ($p in $doc.Paragraphs){ $sz=$null; try{$sz=$p.Range.Font.Size}catch{}; if ($sz -ne $null -and $sz -ne 9999999 -and $sz -lt $min){$min=$sz} }
  L("MIN_FONT_SIZE=$min")
  L('--- section headings (14/bold) ---')
  ...
  foreach ($p in $doc.Paragraphs){ $t=($p.Range.Text -replace "[`r`n`a]","").Trim(); if ($want -contains $t){ $st='';try{$st=$p.Style.NameLocal}catch{}; L('  OK[sz='+$p.Range.Font.Size+' style='+$st+']: '+$t.Substring(0,[Math]::Min(50,$t.Length))) } }
}
catch {
