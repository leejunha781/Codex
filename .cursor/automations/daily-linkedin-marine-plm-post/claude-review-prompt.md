...
| professional-grade | Reference-grade flat vector or premium engineering poster — NOT childish/cartoon |
...
Read `notion-config.json` for database IDs.

Update today's Notion calendar row:
- If both post + image PASS → Status = **Ready**, Image QA = checklist results
- If any FAIL → Status = **Blocked**, Notes = failure reason
