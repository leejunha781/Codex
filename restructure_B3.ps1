...
try {
  ...
  L("old header photo count=" + $t1.Cell(1,2).Range.InlineShapes.Count)

  # locate Profile heading, insert an empty separator paragraph before it (prevents adjacent-table merge)
  $prof=$null
  ...
  $pr=$prof.Range.Duplicate; $pr.Collapse(1); $pr.InsertParagraphBefore()
  L('inserted separator paragraph')

  # re-find Profile heading, insert new table at its start (now separated from old table by the empty para)
  $prof=$null
  foreach($p in $doc.Paragraphs){ if((Norm $p) -eq 'Professional Profile and Interest in ITT Cannon FAE Role'){ $prof=$p; break } }
  $ar=$prof.Range.Duplicate; $ar.Collapse(1)
  $newT=$doc.Tables.Add($ar,6,3)
  L('new table added; cols='+$newT.Columns.Count+' rows='+$newT.Rows.Count)
  if($newT.Columns.Count -ne 3){ throw 'new table merged with old (cols!=3) - abort' }
  ...
  $newT.Cell(1,1).Merge($newT.Cell(1,3)); L('merged title row')
  $newT.Cell(2,3).Merge($newT.Cell(6,3)); L('merged photo column')

  $newT.Cell(2,3).Range.FormattedText = $t1.Cell(1,2).Range.FormattedText
  $imgN=$newT.Cell(2,3).Range.InlineShapes.Count
  L("photo copied; images=$imgN")
  if($imgN -ge 1){ $t1.Delete(); L('old header deleted') } else { L('PHOTO FAIL - kept old') }

  # remove leading empty separator paragraph if present
  try{ $p1=$doc.Paragraphs.Item(1); if((Norm $p1).Length -eq 0){ $rr=$p1.Range.Duplicate; $rr.Delete()|Out-Null; L('removed leading sep para') } }catch{ L('sep remove skip') }

  $h=$doc.Tables.Item(1)
  $h.Range.Font.Name='Arial'; $h.Range.ParagraphFormat.LineSpacingRule=0
  ...
  try{ $h.Columns.Item(1).PreferredWidthType=2; $h.Columns.Item(1).PreferredWidth=[single]24 }catch{ L('c1 fail') }
  try{ $h.Columns.Item(2).PreferredWidthType=2; $h.Columns.Item(2).PreferredWidth=[single]56 }catch{ L('c2 fail') }
  try{ $h.Columns.Item(3).PreferredWidthType=2; $h.Columns.Item(3).PreferredWidth=[single]20 }catch{ L('c3 fail') }

  $tc=$h.Cell(1,1)
  $tc.Range.Font.Size=[single]18; $tc.Range.Font.Bold=[int]1; $tc.Range.Font.Color=[int]$white
  $tc.Range.ParagraphFormat.Alignment=[int]1
  try{ $tc.Shading.BackgroundPatternColor=[int]$charcoal }catch{}
  try{ $tc.VerticalAlignment=[int]1 }catch{}

  for($r=2;$r -le 6;$r++){
    try{ $lc=$h.Cell($r,1); $lc.Range.Font.Size=[single]10; $lc.Range.Font.Bold=[int]1; $lc.Range.Font.Color=[int]$charcoal; $lc.Shading.BackgroundPatternColor=[int]$lightgray; $lc.VerticalAlignment=[int]1 }catch{ L("lab r=$r fail") }
    try{ $vc=$h.Cell($r,2); $vc.Range.Font.Color=[int]$charcoal; $vc.VerticalAlignment=[int]1; if($r -eq 2){ $vc.Range.Font.Size=[single]13; $vc.Range.Font.Bold=[int]1 } else { $vc.Range.Font.Size=[single]11; $vc.Range.Font.Bold=[int]0 } }catch{ L("val r=$r fail") }
  }
  try{ $pc=$h.Cell(2,3); $pc.VerticalAlignment=[int]1; $pc.Range.ParagraphFormat.Alignment=[int]1 }catch{ L('photo align fail') }
  L('styled')
