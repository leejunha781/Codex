...
Save: the final image (PNG) and markdown post go to C:\Users\namma\Documents\Claude\YYYY-MM-DD\<topic-slug> with date/topic filenames (e.g. linkedin-image-YYYY-MM-DD-<topic-slug>.png, linkedin-post-YYYY-MM-DD-<topic-slug>.md). Note: image generation runs in the sandbox, which can only write to the mounted session outputs folder, so generate there first and then copy/save the finished files into the Documents path above; use that Documents path as the image location for the upload step. If copying the binary PNG into the Documents path is not possible from the sandbox, fall back to leaving it in the session outputs folder and report that exact Windows path for the upload. Do not create a separate direct-upload copy in any generated_images folder. Call mcp__cowork__present_files with the final .md and .png.
