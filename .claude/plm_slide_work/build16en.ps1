...
$s = FindLike $pres "E3D / Hull design"
if($s -eq $null){ Write-Output "VER SLIDE NOT FOUND"; $pres.Close(); exit }
$idx = [int]$s.SlideIndex
Write-Output ("ver slide idx=" + $idx)
DelAll $s
$script:slide = $s
...
Txt 26 46 900 16 "The digital thread drives version control - we do not treat 3D design like source code." 11 0 $LIGHT "Aptos" 1 1 | Out-Null

Panel 40 76 880 30 "31240F" $AMBER 1 0.2 | Out-Null
Txt 52 76 110 30 "CHALLENGE" 10 -1 $AMBER "Aptos" 1 3 | Out-Null
Txt 170 76 744 30 "3D files are huge - per-save Git is too heavy  ·  a central server bottlenecks with many concurrent designers  ·  manual upload misses versions" 9.5 0 $LIGHT "Aptos" 1 3 | Out-Null
...
Txt 52 462 90 48 "CORE" 11 -1 $CYAN "Aptos" 1 3 | Out-Null
Txt 150 466 770 44 "E3D is not source code.  DB-element + metadata versioning · content-addressed geometry · versions triggered by promote/baseline events and captured asynchronously - the digital thread drives version control." 10 -1 $WHITE "Aptos" 1 3 | Out-Null

$pres.Save()
$s.Export("$outDir\ver_en.png","PNG",1280,720)
$pres.Close()
[System.Runtime.InteropServices.Marshal]::ReleaseComObject($pres) | Out-Null
[System.Runtime.InteropServices.Marshal]::ReleaseComObject($ppt) | Out-Null
[GC]::Collect(); [GC]::WaitForPendingFinalizers()
Write-Output ("VER_EN_DONE idx=" + $idx)
