...
$deck = Join-Path $proposal 'Future_Industrial_PLM_Meeting_Deck_EN_Final.pptx'
$report = Join-Path (Join-Path $env:USERPROFILE '.claude\plm_slide_work') 'inspect_slide2_v14.txt'
$ppt = $null
$pres = $null
...
try {
    ...
    $pres = $ppt.Presentations.Open($deck, [Microsoft.Office.Core.MsoTriState]::msoTrue, [Microsoft.Office.Core.MsoTriState]::msoFalse, [Microsoft.Office.Core.MsoTriState]::msoFalse)
    $slide = $pres.Slides.Item(2)
    $lines.Add(('Deck: {0}' -f $deck))
    $lines.Add(('Slide count: {0}' -f $pres.Slides.Count))
    $lines.Add(('Slide 2 shapes: {0}' -f $slide.Shapes.Count))
