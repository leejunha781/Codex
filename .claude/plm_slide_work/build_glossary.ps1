...
$NAVY   = 3678739      # #132238
$WHITE  = 16777215
$LIGHT  = 15062727     # #C7D6E5 definition text
$MUTED  = 12626067     # #93A8C0 brand / page num
$CYAN   = 15646772     # #34C0EE
$AMBER  = 3846888      # #E8B23A
$GREEN  = 9224777      # #49C28C
$DIVID  = 6178356      # #34465E
...
function Build-Glossary($idx,$pageno,$title,$colAhdr,$colAcol,$colA,$colBhdr,$colBcol,$colB){
  ...
  $sub = "Appendix " + [char]0x00B7 + " quick reference " + [char]0x2014 + " plain-language key to the abbreviations used throughout this deck, for the development and planning teams."
  ...
  for($i=0;$i -lt $colA.Count;$i++){
    $p=$colA[$i].Split("|||",[System.StringSplitOptions]::RemoveEmptyEntries)
    $y=$startY + $i*$stepA
    Add-Entry $slide 46 $y 432 ($stepA) $p[0] $p[1] $colAcol | Out-Null
  }
  $stepB = [int]((498-$startY)/$colB.Count)
  for($i=0;$i -lt $colB.Count;$i++){
    $p=$colB[$i].Split("|||",[System.StringSplitOptions]::RemoveEmptyEntries)
    $y=$startY + $i*$stepB
    Add-Entry $slide 496 $y 430 ($stepB) $p[0] $p[1] $colBcol | Out-Null
  }

  $foot = "Quick-reference appendix " + [char]0x00B7 + " abbreviations are also defined where first used in the deck " + [char]0x00B7 + " product names are trademarks of their respective owners."
...
Build-Glossary 31 31 ("Glossary  1 / 2   " + [char]0x2014 + "   PLM, Engineering & Marine terms") `
  "PLM " + [char]0x00B7 + " CONFIGURATION " + [char]0x00B7 + " CHANGE" $CYAN $g1A `
  "MARINE " + [char]0x00B7 + " NAVAL " + [char]0x00B7 + " CLASS" $AMBER $g1B | Out-Null
...
Build-Glossary 32 32 ("Glossary  2 / 2   " + [char]0x2014 + "   Products, Architecture, Security & Infrastructure") `
  "PRODUCTS " + [char]0x00B7 + " PLATFORMS" $GREEN $g2A `
  "ARCHITECTURE " + [char]0x00B7 + " SECURITY " + [char]0x00B7 + " INFRA" $CYAN $g2B | Out-Null

$pres.Save()
Write-Output ("SAVED. Total slides now: " + $pres.Slides.Count)
$pres.Close()
