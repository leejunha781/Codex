...
Working folder `D:\이력서\모아컴코리아 - HW 개발\`. Files: `이준하_모아컴 코리아_HW 개발.docx` = **양식 기준** (NOTE: its body text is the old 쏠리드 위성통신 version — layout reference only); `..._HW설계_최종.docx` = user's MoaComm draft; `..._HW설계_최종_v2.docx` = my revised version (2026-06-10, 14 pp). **CANONICAL SUBMISSION = `이준하_모아컴코리아_HW 설계.docx` (+pdf)** — the user took my v2, edited it themselves (08:19): rebuilt the 연봉 table (희망연봉 reverted to their preferred "회사 보상 밴드 및 직무·역량 수준에 따라 협의"; 현재연봉 itemized 기본급 8,397 / 변동상여 677 / **고정상여 360 (corrected from 377 — components now sum exactly to the old 총 9,484만원)** / 복지포인트 50만원; dropped 성과급·입사시기 columns), added page-number footer. Recruiter contact: **권영웅** (search firm, likely J&C HR) — asked for free-format resume with accurate 핵심역량/보유기술/현재연봉, values RF 장비 HW 개발 at 통신장비 기업; I drafted the Korean reply email (2026-06-10) quoting the itemized salary. Format rules from user: 양식 변형 금지, **맑은 고딕, 한줄 간격(lsr=0)**, formal/strong non-AI tone. Scripts/QA in `C:\Users\namma\.claude\moacom_work\` (extract/edit/render .ps1 + PNG renders).
